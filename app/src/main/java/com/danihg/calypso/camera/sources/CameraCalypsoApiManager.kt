package com.danihg.calypso.camera.sources
...
class CameraCalypsoApiManager(context: Context) : CameraDevice.StateCallback() {
    ...
    /**
     * Establece un valor de ISO manual (sensibilidad).
     * Internamente deshabilita el AE y fija la sensibilidad al rango soportado.
     * Devuelve true si la petición se aplicó correctamente.
     */
    fun setManualISO(iso: Int): Boolean {
        val characteristics = cameraCharacteristics ?: return false
        // Rango disponible de sensibilidad del sensor
        val sensitivityRange: Range<Int> =
            characteristics.get(CameraCharacteristics.SENSOR_INFO_SENSITIVITY_RANGE)
                ?: return false

        // Coercion para que el ISO esté dentro del rango válido
        val isoValue = iso.coerceIn(sensitivityRange.lower, sensitivityRange.upper)

        val builder = builderInputSurface ?: return false

        // 1) Deshabilitar AE (autoexposición)
        builder.set(CaptureRequest.CONTROL_AE_MODE, CaptureRequest.CONTROL_AE_MODE_OFF)
        // 2) Fijar la sensibilidad manual
        builder.set(CaptureRequest.SENSOR_SENSITIVITY, isoValue)

        // Actualizar flag interno de “auto exposición” a false
        isAutoExposureEnabled = false
    ...
    /**
     * Lee el ISO actual que está en el CaptureRequest.Builder (0 si no está configurado).
     */
    fun getCurrentISO(): Int {
        val builder = builderInputSurface ?: return 0
        // Si nunca se ha fijado manual, puede devolver null: en ese caso devolvemos 0.
        return builder.get(CaptureRequest.SENSOR_SENSITIVITY) ?: 0
    }

    /**
     * Reactiva la autoexposición (y por tanto el ISO vuelve a gestionarse automáticamente).
     * Devuelve true si el modo AE_ON está disponible y se aplicó.
     */
    fun enableAutoISO(): Boolean {
        val builder = builderInputSurface ?: return false
        val characteristics = cameraCharacteristics ?: return false

        // Verificar que el modo AE_ON esté soportado
        val aeModes: IntArray? =
            characteristics.get(CameraCharacteristics.CONTROL_AE_AVAILABLE_MODES)
        if (aeModes == null || !aeModes.contains(CaptureRequest.CONTROL_AE_MODE_ON)) {
            return false
        }

        // Poner CONTROL_AE_MODE en ON (auto exposición)
    ...
    /**
     * Indica si actualmente estamos en modo “auto ISO” (i.e. si el AE está activo).
     */
    fun isAutoISO(): Boolean {
        return isAutoExposureEnabled
    }

    /**
     * Ajusta la compensación de exposición (EV steps). Sólo funciona si
     * AE está soportado (se reactivar AE en ON).
     * @param evSteps valor de compensación (por ejemplo, ±5 pasos máximos)
     * @return true si se aplicó correctamente
     */
    fun setExposureCompensation(evSteps: Int): Boolean {
        val characteristics = cameraCharacteristics ?: return false
        val builder = builderInputSurface ?: return false

        // 1) Obtenemos el rango que soporta la cámara
        val compRange: Range<Int> =
            characteristics.get(CameraCharacteristics.CONTROL_AE_COMPENSATION_RANGE)
                ?: return false

        // 2) Coercion a los valores reales del hardware
        val evClamped = evSteps.coerceIn(compRange.lower, compRange.upper)

        // 3) Aseguramos que AE está en ON
        builder.set(CaptureRequest.CONTROL_AE_MODE, CaptureRequest.CONTROL_AE_MODE_ON)
        // 4) Fijamos la compensación
        builder.set(CaptureRequest.CONTROL_AE_EXPOSURE_COMPENSATION, evClamped)

        // 5) Actualizamos el flag interno
        isAutoExposureEnabled = applyRequest(builder)
        return isAutoExposureEnabled
    }

    /**
     * @return compensación de exposición actual (EV steps) o 0 si no está disponible
     */
    fun getExposureCompensation(): Int {
        val builder = builderInputSurface ?: return 0
        return builder.get(CaptureRequest.CONTROL_AE_EXPOSURE_COMPENSATION) ?: 0
    }

    /**
     * Devuelve el rango (min..max) de índices de compensación AE
     */
    val exposureCompensationRange: Range<Int>
        get() {
            val characteristics = cameraCharacteristics ?: return Range(0, 0)
            return characteristics.get(CameraCharacteristics.CONTROL_AE_COMPENSATION_RANGE)
                ?: Range(0, 0)
        }

    /**
     * Fija un tiempo de exposición manual (en nanosegundos), desactivando AE.
     * @param exposureTimeNs tiempo en ns (por ejemplo, 33_333_333 para 1/30 s)
     * @return true si se aplicó correctamente
     */
    fun setManualExposureTime(exposureTimeNs: Long): Boolean {
        val characteristics = cameraCharacteristics ?: return false
        val builder = builderInputSurface ?: return false

        // 1) Rango de tiempos de exposición soportado
        val expRange: Range<Long> =
            characteristics.get(CameraCharacteristics.SENSOR_INFO_EXPOSURE_TIME_RANGE)
                ?: return false

        // 2) Coercion a lo que realmente soporte el hardware
        val tClamped = exposureTimeNs.coerceIn(expRange.lower, expRange.upper)

        // 3) Desactivar AE
        builder.set(CaptureRequest.CONTROL_AE_MODE, CaptureRequest.CONTROL_AE_MODE_OFF)
        // 4) Fijar el tiempo de exposición
        builder.set(CaptureRequest.SENSOR_EXPOSURE_TIME, tClamped)

        // 5) Actualizar flag interno
        isAutoExposureEnabled = false
    ...
    /**
     * @return tiempo de exposición manual (ns) actualmente fijado, o 0 si no hay
     */
    fun getCurrentExposureTime(): Long {
        val builder = builderInputSurface ?: return 0L
        return builder.get(CaptureRequest.SENSOR_EXPOSURE_TIME) ?: 0L
    }
