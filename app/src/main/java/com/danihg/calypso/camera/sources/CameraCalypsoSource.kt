...
class CameraCalypsoSource(context: Context): VideoSource() {
    ...
    /**
     * Fija un valor de ISO manual. Devuelve true si se aplicó correctamente.
     */
    fun setISO(value: Int): Boolean {
        if (isRunning()) {
            return camera.setManualISO(value)
        }
        return false
    }

    /**
     * Devuelve el ISO actual (si no se ha fijado manual, puede devolver 0).
     */
    fun getISO(): Int {
        return if (isRunning()) {
            camera.getCurrentISO()
        } else {
            0
        }
    }
    ...
    /**
     * Reactiva el ISO automático (es decir, vuelve a poner AE en modo AUTO).
     * Devuelve true si el cambio se aplicó.
     */
    fun enableAutoISO(): Boolean {
        if (isRunning()) {
            return camera.enableAutoISO()
        }
        return false
    }

    /**
     * Indica si estamos en modo ‘auto ISO’ (equivale a que el AE esté habilitado).
     */
    fun isAutoISOEnabled(): Boolean {
        return if (isRunning()) {
            camera.isAutoISO()
        } else {
            false
    ...
    fun getMinISO(): Int {
        ...
        val characteristics = camera.cameraCharacteristics ?: return 0
    ...
    /**
     * Retorna el valor máximo de ISO soportado por el hardware (o 0 si no está corriendo).
     */
    fun getMaxISO(): Int {
        if (!isRunning()) return 0
        val characteristics = camera.cameraCharacteristics ?: return 0
    ...
    /**
     * @see CameraCalypsoApiManager.setExposureCompensation
     */
    fun setExposureCompensation(evSteps: Int): Boolean {
        return if (isRunning()) {
            camera.setExposureCompensation(evSteps)
        } else {
            false
        }
    }
    ...
    /**
     * @see CameraCalypsoApiManager.getExposureCompensation
     */
    ...
    fun getMinExposureCompensation(): Int {
        // Si el builder no existe o no está corriendo, devolvemos 0 por seguridad
        if (!isRunning()) return 0
        val characteristics = camera.cameraCharacteristics ?: return 0
        return characteristics.get(CameraCharacteristics.CONTROL_AE_COMPENSATION_RANGE)?.lower ?: 0
    }
    fun getMaxExposureCompensation(): Int {
        if (!isRunning()) return 0
        val characteristics = camera.cameraCharacteristics ?: return 0
        return characteristics.get(CameraCharacteristics.CONTROL_AE_COMPENSATION_RANGE)?.upper ?: 0
    }

    /**
     * @see CameraCalypsoApiManager.setManualExposureTime
     */
    fun setExposureTime(exposureTimeNs: Long): Boolean {
        return if (isRunning()) {
            camera.setManualExposureTime(exposureTimeNs)
        } else {
            false
        }
    }
    ...
    /**
     * @see CameraCalypsoApiManager.getCurrentExposureTime
     */
    fun getExposureTime(): Long {
        return if (isRunning()) {
            camera.getCurrentExposureTime()
        } else {
            0L
        }
    }
    ...
    fun addImageListener(format: Int, maxImages: Int, autoClose: Boolean = true, listener: ImageCallback) {
        ...
        camera.addImageListener(w, h, format, maxImages, autoClose,
            listener as CameraCalypsoApiManager.ImageCallback
        )
    }
